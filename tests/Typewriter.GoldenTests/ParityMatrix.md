...
| Nullable types | `identical` | Nullable reference/value type handling (`T?` → `T \| null`) matches upstream behavior | `complex-types` (PropertyInfo nullable properties, MethodInfo `Task<int?>`) |
| Generic types | `identical` | `$TypeArguments`, `$TypeParameters`, nested generic unwrapping (`Task<IHttpActionResult<T[]>>`) match upstream | `complex-types` (GenericClassInfo, GenericPropertyInfo, GenericMethodInfo, IGenericInterface) |
| Partial classes | `identical` | Members from all partial declarations are merged into a single code-model class, matching upstream | `complex-types` (PartialClassInfo split across two files) |
...
| Collision naming | `identical` | When multiple input types produce the same output filename, `_1`/`_2` suffixes are appended, matching upstream `Template.GetOutputFilename` collision avoidance | `complex-types` (multiple ClassInfo variants producing same base filename) |
...
| `source-generators` | `tests/fixtures/SourceGenerators/` | Source generator symbol visibility; `$Classes` on generated types |
| `complex-types` | `tests/fixtures/complex-types/` (planned) | Nullable types; generic types; partial classes; collision naming; requestRender batch mode |
